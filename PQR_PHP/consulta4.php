...
if (isset($_SESSION['db_servername'], $_SESSION['db_username'], $_SESSION['db_password'], $_SESSION['db_databaseName'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['pesos'])) {
        $pesos = $conn->real_escape_string($_POST['pesos']);
        ...
        $sql = " SELECT 
            ...
            CASE 
                WHEN padecimiento.nombrePadecimiento IN ('Cancer', 'Enfermedad cardiaca', 'EPOC', 'Hepatitis', 'Renal') THEN 'Grave'
                WHEN padecimiento.nombrePadecimiento IN ('Hipertension', 'Diabetes', 'Asma', 'Artritis', 'Osteoporosis') THEN 'Moderado'
        ...
        INNER JOIN 
            ...
            WHERE datospersonales.nombre = '$pesos'
        ORDER BY 
